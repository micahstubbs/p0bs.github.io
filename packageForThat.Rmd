...
Here's a page that I've prepared for an R training course that I ran at my company. The idea was to show that just as Apple has an app for that, so too does R (although they're called packages).
...
(Note that I've dimmed-out much of this code below, so that I can show it on this static webpage. However, I ran the dimmed-out code separately and saved the ensuing results as an R Dataset. I can then just load this dataset for using in this static webpage.)
...
To understand this data even more, it would help to map it. And the amount of  boundary files available for UK maps is extraordinary. R helps us to use these maps, so that we can map each local authority (the areas over which the referendum votes were amalgamated). (To speed-up the code, I've also done as before and run the code in advance, saving the RDS, only to load it for this page.)
...
We could use the polygon details that we have in our data to map the data. However, circles on a map would enable us to alter the size of the circle and therefore show more detail about the data. As such, we'll use R to calculate the centre points of each local authority.
...
We're then ready to plot the data. When we do so, some regional trends are immediately apparent.
...
And that's it! Hopefully, this has given you a quick appreciation of what R can help you to achieve fairly quickly.
